...
async function main() {
  ...
  try {
    ...
    const configPath = path.join(process.cwd(), 'config', 'juiceshop-full.config.json');
    ...
    const passiveDetectors = registry.getPassiveDetectors();
    ...
    // 5. Run Scan
    logger.info('Starting full Juice Shop scan...');
