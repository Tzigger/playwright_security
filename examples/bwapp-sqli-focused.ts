...
async function runSqliScenario(pages: PageTarget[]) {
  ...
  const logger = new Logger(LogLevel.INFO, 'bwapp-sqli');
  ...
  console.log(`SQLi focused scan complete. Findings: ${result.vulnerabilities.length}`);
...
(async () => {
  const pages: PageTarget[] = [
    ...
    { url: '/sqli_6.php', name: 'SQLi Login Bypass' },
    { url: '/sqli_10-1.php', name: 'SQLi AJAX/JSON' },
  ];

  await runSqliScenario(pages);
})().catch((err) => {
  console.error('SQLi focused scan failed', err);
  process.exit(1);
});
